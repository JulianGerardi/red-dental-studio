      ...
      >
        <div className="flex flex-col gap-8">
          {gruposDeTokens().map(({ grupo, filas }) => (
            <div key={grupo}>
              <h3 className="mb-2 text-[13px] font-semibold text-[#52525b]">{grupo}</h3>
              <div className="overflow-hidden rounded-lg border border-[#e4e4e7]">
                <table className="w-full text-left text-[12.5px]">
                  ...
                  <tbody>
                    {filas.map((f) => (
                      <tr key={f.nombre} className="border-t border-[#f1f1f4]">
                        <td className="px-3 py-2"><Muestra color={aCss(f.claro)} /></td>
                        <td className="px-3 py-2">{f.oscuro ? <Muestra color={aCss(f.oscuro)} /> : <span className="text-[#a1a1aa]">-</span>}</td>
                        <td className="px-3 py-2"><Codigo>--{f.nombre}</Codigo></td>
                        <td className="px-3 py-2"><Codigo>{utilidad(f.nombre)}</Codigo></td>
                        <td className="px-3 py-2 font-mono text-[12px] text-[#52525b]">
                          {hslAHex(f.claro) ?? f.claro}
                          {f.oscuro && <span className="text-[#a1a1aa]"> · {hslAHex(f.oscuro) ?? f.oscuro}</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      >
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {coloresDeTema.map(({ nombre, valor }) => (
            <div key={nombre} className="flex items-center gap-3 rounded-lg border border-[#e4e4e7] p-3">
              <Muestra color={`var(--${nombre})`} />
              <div className="min-w-0">
                <p className="truncate font-mono text-[12px] font-medium">--{nombre}</p>
                <p className="truncate text-[11.5px] text-[#71717a]">bg-{nombre.replace('color-', '')} · {valor}</p>
              </div>
            </div>
          ))}
        </div>
