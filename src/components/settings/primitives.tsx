...
export function Card({ title, children, className }: { title?: string; children: React.ReactNode; className?: string }) {
  return (
    <section className={cn('rounded-xl border border-[#e4e4e7] bg-white p-4 sm:p-5', className)}>
      {title && <h2 className="text-sm font-bold text-[#09090b]">{title}</h2>}
...
export function Toggle({ label, on, onChange }: { label?: string; on: boolean; onChange: (v: boolean) => void }) {
  return (
    <div className="flex flex-col gap-2">
      {label && <span className="text-xs font-medium text-[#09090b]">{label}</span>}
      <button
        type="button"
        role="switch"
        aria-checked={on}
