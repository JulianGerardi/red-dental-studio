...
/* Preview del consentimiento como lo recibe el paciente: una hoja de papel
   sobre el escritorio gris del panel, no una tarjeta más de la UI. Detalle en
...
const ETIQUETA = 'font-sans text-[9px] font-bold tracking-[0.12em] uppercase'

function Seccion({ titulo, children }: { titulo: string; children: React.ReactNode }) {
  return (
    <section className="mt-3.5">
      <h4 className={`${ETIQUETA} border-b border-[#e4e4e7] pb-1 text-[#52525b]`}>{titulo}</h4>
...
function Campo({ icono: Icono, etiqueta, children }: { icono: LucideIcon; etiqueta: string; children: React.ReactNode }) {
  return (
    <div className="bg-white p-2">
      ...
      <div className="mt-1 font-sans text-[10.5px] leading-snug">{children}</div>
    </div>
  )
}
...
export function ConsentDocument({ titulo, procedimiento, naturaleza, riesgos, vistaPaciente }: Props) {
  ...
  const vacio = <p className="text-[#a1a1aa] italic">No content yet.</p>

  return (
    <article
      aria-label="Consent document preview"
      className="mx-auto flex aspect-[8.5/11] w-full max-w-[400px] flex-col bg-white px-5 pt-5 pb-3 font-serif text-[11px] leading-[1.55] text-[#27272a] shadow-[0_0_0_1px_rgb(0_0_0/0.06),0_2px_4px_rgb(0_0_0/0.06),0_14px_28px_-8px_rgb(0_0_0/0.22)]"
    >
      <header className="flex items-center gap-2.5 border-b-2 border-[#1d56bc] pb-2.5 font-sans">
        <span aria-hidden className="bg-dash-blue grid size-8 shrink-0 place-items-center rounded-md text-[11px] font-bold text-white">LA</span>
        <div className="flex min-w-0 flex-col gap-0.5 text-[10px] text-[#71717a]">
      ...
      <div className="mt-4 text-center">
        <p className={`${ETIQUETA} tracking-[0.18em] text-[#1d56bc]`}>Informed Consent</p>
        <h3 className="mt-1 text-[16px] leading-tight font-bold text-[#09090b]">{titulo || 'Untitled Consent'}</h3>
      </div>

      <div className="mt-3.5 grid grid-cols-2 gap-px border border-[#d4d4d8] bg-[#d4d4d8]">
        ...
        <ul className="flex flex-col gap-1">
          {RECONOCIMIENTOS_PACIENTE.map((r) => (
            <li key={r} className="flex gap-2">
              <span aria-hidden className="mt-[3px] size-2.5 shrink-0 rounded-[2px] border border-[#71717a]" />
              {r}
            </li>
          ))}
        </ul>
      ...
      <div className="mt-auto grid grid-cols-[1fr_88px] gap-4 pt-8 font-sans text-[9.5px] text-[#71717a]">
      ...
      <footer className="mt-3 flex justify-between border-t border-[#efeff2] pt-1.5 font-sans text-[9px] text-[#a1a1aa]">
