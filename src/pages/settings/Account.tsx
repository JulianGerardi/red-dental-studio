...
import { CreditCard, Crown, ReceiptText, Building2 } from 'lucide-react'
...
/* Settings → Account. Ver red.dev.confidentally.com/settings/account
   ...
   Dos entradas, un solo componente:
   - /settings/account -"Profile" en el menú de cuenta (Topbar.tsx)-: la
     cuenta propia del usuario logueado.
   - /settings/accounts/:accountId -el nombre de una fila en Settings →
     Accounts.tsx es un link a esto-: Julián marcó que en el sitio real
     entrar a una cuenta de esa lista lleva a SU configuración, con las
     mismas tres pestañas. `useParams` decide cuál mostrar; sin id es la
     propia.

   Mismo patrón de tabs que Employees.tsx (la ficha de un empleado, la
   pantalla que Julián señaló como "el estilo de roles"): título + bajada,
   tira de pestañas en `bg-[#f1f5f9]`, contenido en Card, un solo Cancel/Save
   al pie de toda la pantalla.
   ...
   existe en Employees.tsx y Relationships, para la cuenta propia -una
   persona-. Para una cuenta de la lista -una clínica, no una persona- ese
   trío no aplica igual: General Information se reduce a nombre de la
   práctica + email, sin apellido ni fecha de nacimiento.

   Subscription y Owner no tienen precedente propio en este código: el sitio
   real está atrás de un lock de una sola pestaña que no pude destrabar en
   esta sesión. Para la cuenta propia son la mejor reconstrucción con el
   vocabulario que ya usa Settings → Accounts (plan, vencimiento, estado,
   licencias) -a confirmar contra el sitio real apenas se pueda entrar-. Para
   una cuenta de la lista, en cambio, SÍ hay datos reales de ese mismo
   `CUENTAS`: se usan tal cual, incluidos los guiones de cuentas sin plan,
   sin dueño o sin licencias -no se les inventa un valor, mismo criterio que
   ya sigue Accounts.tsx-. */
...
/* TextField/SelectField sólo quedan controlados -y muestran el `value` que
   ...
   importa que se vea la información precargada, así que cada campo tiene su
   estado en vez de un `value` suelto. */
export function SettingsAccount() {
  const { accountId } = useParams()
  const cuenta = accountId ? CUENTAS.find((c) => c.id === accountId) : undefined

  const [tab, setTab] = useState<Tab>('Information')
  const [foto, setFoto] = usePhoto(cuenta ? `account-photo:${cuenta.id}` : 'account-photo:me')

  const [general, setGeneral] = useState<{ first?: string; last?: string; email: string; practica?: string }>(() => (
    cuenta
      ? { practica: cuenta.nombre, email: '' }
      : { first: 'Sarah', last: 'Stone', email: 'sarah.stone@reddentalstudio.com' }
  ))
  const [contacto, setContacto] = useState({ codigo: '+1', numero: cuenta ? '' : '(555) 234-5678', extension: '' })
  const [domicilio, setDomicilio] = useState({
    linea1: cuenta ? '' : '123 Biscayne Blvd', linea2: '', pais: cuenta ? '' : 'United States',
    region: cuenta ? '' : 'Florida', ciudad: cuenta ? '' : 'Miami', cp: cuenta ? '' : '33101',
  })
  ...
  return (
    <div className="px-4 py-6 sm:px-8">
      ...
      <div className="mt-4 flex flex-col gap-4">
          ...
          <>
            <Card>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span>
                  <span className="block text-xl font-bold text-[#09090b]">{cuenta ? cuenta.nombre : 'Sarah Stone'}</span>
                  <Pill tone={cuenta ? ESTADO_TONO_CUENTA[cuenta.estado] : 'success'} className="mt-1">
                    {cuenta ? cuenta.estado : 'Dentist'}
                  </Pill>
                </span>
              </div>
              ...
              {cuenta ? (
                <div className="mt-3 grid gap-4 sm:grid-cols-2">
                  <TextField
                    ...
                    value={general.practica ?? ''}
                    onChange={(v) => setGeneral((g) => ({ ...g, practica: v }))}
                  />
                  <TextField
                    label="Email" placeholder="Placeholder"
                    value={general.email}
                    onChange={(v) => setGeneral((g) => ({ ...g, email: v }))}
                  />
                </div>
              ) : (
                <div className="mt-3 grid gap-4 sm:grid-cols-2">
            ...
            <Card title="Contact Information">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <TextField label="Number" required placeholder="Placeholder" value={contacto.numero} onChange={(v) => setContacto((c) => ({ ...c, numero: v }))} />
              </div>
            ...
            <Card title="Subscription Summary">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-lg border border-[#e4e4e7] p-3.5">
                  ...
                  <span className="mt-1.5 block text-[15px] font-bold text-[#09090b]">
                    {cuenta ? cuenta.plan : 'Confidentally Premium'}
                  </span>
                </div>
                <div className="rounded-lg border border-[#e4e4e7] p-3.5">
                  ...
                  <span className="mt-1.5 block">
                    {cuenta ? (
                      cuenta.estadoSuscripcion && cuenta.estadoSuscripcion in ESTADO_TONO ? (
                        <Pill tone={ESTADO_TONO[cuenta.estadoSuscripcion as EstadoSuscripcion]}>{cuenta.estadoSuscripcion}</Pill>
                      ) : (
                        <span className="text-[15px] font-bold text-[#a1a1aa]">—</span>
                      )
                    ) : (
                ...
                <div className="rounded-lg border border-[#e4e4e7] p-3.5">
                  ...
                  <span className="mt-1.5 block text-[15px] font-bold text-[#09090b] tabular-nums">
                    {cuenta ? (cuenta.vence || '—') : 'Sep 30, 2027'}
                  </span>
                </div>
                <div className="rounded-lg border border-[#e4e4e7] p-3.5">
                  <span className="text-[11px] font-semibold tracking-wide text-[#71717a] uppercase">Licenses</span>
                  <span className="mt-1.5 block text-[15px] font-bold text-[#09090b] tabular-nums">
                    {cuenta ? cuenta.licencias : '12 / 15 used'}
                  </span>
                </div>
              </div>
              <button
            ...
            <Card title="Subscription History">
              {cuenta ? (
                <EmptyState
                  ...
                  detail="This account doesn't have a subscription history yet."
                  className="border-0"
                />
              ) : (
        ...
        {tab === 'Owner' && (
          <Card>
            {cuenta && cuenta.duenos === '—' ? (
              <EmptyState
                icon={Crown}
                title="No owner on file"
                detail="This account doesn't have a practice owner assigned yet."
                className="border-0"
              />
            ) : (
              <>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span className="bg-dash-count-bg text-dash-blue-hover flex size-14 shrink-0 items-center justify-center rounded-lg text-base font-semibold">
                      {iniciales(cuenta ? cuenta.duenos : duenio.nombre)}
                    </span>
                    <span>
                      <span className="block text-xl font-bold text-[#09090b]">{cuenta ? cuenta.duenos : duenio.nombre}</span>
                ...
                {cuenta && (
                  <p className="mt-3 flex items-center gap-1.5 text-[12px] text-[#71717a]">
